...
internal class PolicyCacheTest {
    ...
    private lateinit var rbacOrganizations: Organizations
    ...
    fun `throws TenancyException when subjectOrgId does not match authZ request organizationId`() {
        val policyCache = PolicyCache(rbac, rbacOrganizations, testScope)
    ...
    fun `fetches policy on first authorization check`() {
        ...
        val policyCache = PolicyCache(rbacMock, rbacOrganizations, testScope)
    ...
    fun `fetches org policy on first authorization check for an org`() {
        ...
        val policyCache = PolicyCache(rbac, rbacOrgMock, testScope)
